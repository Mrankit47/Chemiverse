...
function Chip({ label, color }) {
  return (
    <span className="px-4 py-2 rounded-xl font-mono font-semibold text-sm glass" style={{ borderColor: color, color }}>
      {label}
    </span>
  );
}
    ...
    >
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="glass rounded-2xl p-4 h-fit">
          <div className="text-xs text-[var(--muted)] uppercase tracking-wider mb-3 px-2">Reactions</div>
            ...
            {REACTIONS.map((r) => (
              <button
                ...
                className={`w-full text-left px-3 py-3 rounded-xl transition-all ${
                  active.id === r.id ? "bg-white/10 text-white" : "hover:bg-white/5 text-[var(--muted)]"
                }`}
              >
                <div className="font-medium text-sm">{r.name}</div>
                <div className="font-mono text-xs mt-1" style={{ color: r.color }}>{r.type}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="glass rounded-3xl p-8 min-h-[320px] flex flex-col items-center justify-center relative overflow-hidden">
                  ...
                  >
                    {active.reactants.map((r, i) => (
                      <React.Fragment key={i}>
                        ...
                        {i < active.reactants.length - 1 && <span className="text-2xl text-[var(--muted)]">+</span>}
                      </React.Fragment>
                    ))}
                  </motion.div>
                ) : (
                  ...
                  >
                    {active.products.map((p, i) => (
                      <React.Fragment key={i}>
                        ...
                        {i < active.products.length - 1 && <span className="text-2xl text-[var(--muted)]">+</span>}
                      </React.Fragment>
                    ))}
                  </motion.div>
                )}
            ...
            <div className="relative mt-8 font-mono text-lg text-center" data-testid="reaction-equation">
              {active.equation}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-3 gap-4 mt-5">
            <div className="glass rounded-2xl p-5 sm:col-span-2">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-semibold capitalize">{active.energy}</span>
              </div>
              <p className="text-[var(--muted)] text-sm leading-relaxed">{active.desc}</p>
            </div>
            <div className="flex flex-col gap-3">
              <button
                ...
                className="flex-1 rounded-2xl font-semibold bg-[var(--cyan)] text-black hover:glow-cyan transition-all inline-flex items-center justify-center gap-2 disabled:opacity-50"
              >
              ...
              <button
                ...
                onClick={reset}
                className="rounded-2xl font-semibold glass py-3 inline-flex items-center justify-center gap-2 hover:border-[var(--cyan)]"
              >
