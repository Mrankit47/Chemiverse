...
# categories: alkali, alkaline, transition, post-transition, metalloid,
...
def shells(n):
    caps = [2,8,18,32,32,18,8]
    res=[]; rem=n
    for c in caps:
        if rem<=0: break
        take=min(c,rem); res.append(take); rem-=take
    return res

# lanthanide/actinide column positions (f-block), placed row-wise
f_pos = {}
for i,num in enumerate(range(58,72)): f_pos[num]=i+3   # Ce..Lu columns 3..16
for i,num in enumerate(range(90,104)): f_pos[num]=i+3

out=[]
for (num,sym,name,mass,cat,grp,per) in E:
    if cat=="lanthanide" and num>=58:
        row=9; col=f_pos.get(num,3)
    elif cat=="actinide" and num>=90:
        row=10; col=f_pos.get(num,3)
    else:
        row=per; col=grp
    out.append({
        "number":num,"symbol":sym,"name":name,"mass":mass,
        "category":cat,"group":grp,"period":per,
        "row":row,"col":col,"shells":shells(num),
    })

with open("/app/frontend/src/data/elements.json","w") as f:
    json.dump(out,f)
